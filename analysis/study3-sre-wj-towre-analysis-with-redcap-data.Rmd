...
df.legacy.sre.raw <- read.csv("~/Documents/roar-analysis/data/prod_legacy_sre_raw_trials.csv")

df.va.wj.raw <- read.csv("~/Documents/roar-analysis/data/visualAttentionSRFValidationData.csv") %>%
  mutate(mffScore = NA) %>%
  select(pid, "years" = visit_age_yrs, "grade" = visit_grade, 
         "srfCorrect" = wj_srf_correct_raw, "srfIncorrect" = wj_srf_incorrect_raw,
         mffScore)

df.stanford.wj.raw <- read.csv("~/Documents/roar-analysis/data/lmb_F1_C3_wj_srf_data.csv") %>%
  select(-redcapId) %>%
  rbind(df.va.wj.raw)
```
...
sre.wj.int.plot <- ggplot(df.sre.srf.int %>% 
  ...
  labs(x = "SRE time elapsed (s)",
       y = "Correlation of SRE-Human & WJ-SRF") +
