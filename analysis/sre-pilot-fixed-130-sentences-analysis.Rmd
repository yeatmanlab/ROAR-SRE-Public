---
title: "ROAR-SRE-Pilot Fixed Items Analysis & Figures"
author: "Jasmine Tran"
date:  "`r Sys.Date()`"
output:
...
```{r, include = FALSE}
df.resp.matrix.raw <- read.csv("~/Documents/roar-analysis/data/sre-paper-data/sft_response_pattern_w_lab_w_kipp.csv")
df.rt.matrix.raw <- read.csv("~/Documents/roar-analysis/data/sre-paper-data/sft_response_time_w_lab_w_kipp.csv")
```
...
df.resp.matrix.lab <- df.resp.matrix.raw %>%
  select(subj, starts_with("lab")) %>% # filtering based on the lab sentences ONLY
  mutate(correct = rowSums(df.resp.matrix.raw[2:lab.length], na.rm = T),
         incorrect = rowSums(df.resp.matrix.raw[2:lab.length] == 0, na.rm = T),
...
df.resp.matrix.tosrec <- df.resp.matrix.raw %>%
  select(subj, starts_with("TOSREC")) %>% # filtering based on the lab sentences ONLY
  mutate(correct = rowSums(df.resp.matrix.raw[2:lab.length], na.rm = T),
         incorrect = rowSums(df.resp.matrix.raw[2:lab.length] == 0, na.rm = T),
         sum = correct + incorrect, 
         final = correct - incorrect,
...
We filtered out items that had a proportion correct of 85% or greater for our intial pass of the stimuli bank.
```{r, include = FALSE}
...
```{r, include = FALSE}
# looking at avg RT against agreeability (pCorr) + analyzing against sentence length
...
```{r, include = FALSE}
df.lab.items.filtered.v3
```

## Inspecting relationship between agreeability and average reaction time (RT)
```{r}
ggplot(df.lab.items.filtered.v3, mapping = aes(x = avgRTStd,
                     y = pCorrStd, color=numCharacter)) + 
  geom_point() + 
  labs(title = "Response Time vs. Agreeability Rate",
...
```{r}
df.lab.items.filtered.v4 <- df.lab.items.filtered.v3 %>%
...
df.lab.items.filtered.difficulty <- df.lab.items.filtered.v4 %>%
...
df.lab.items.easy <- df.lab.items.filtered.difficulty %>%
  filter(difficulty == "Easy", sentenceId != "lab_196")
```
