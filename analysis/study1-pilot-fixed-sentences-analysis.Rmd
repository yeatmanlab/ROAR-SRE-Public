...
df.filtered.plot <- df.lab.items.filtered.v1 %>%
  mutate(Filtered = ifelse(pCorr > 0.85, "True", "False"))
...
filtered.plot <- ggplot(data = df.filtered.plot, label = sentence, aes(label = sentence)) + 
  ...
  scale_x_continuous("Average response time (seconds)") +
  scale_y_continuous("Agreement rate (proportion)") +
...
quad.plot <- ggplot(data = df.quad.plot, label = sentence, aes(label = sentence)) + 
  ...
  geom_label_repel(data = df.quad.plot %>% 
                   ...
                   fill = "gray95", family="Avenir", size = 2.5) +
  scale_x_continuous(limits = c(-3, 3), breaks = round(seq(-3,3)), "Average response time (seconds)") +
  scale_y_continuous(limits = c(-3, 3), breaks = round(seq(-3,3)),"Average agreement rate (proportion)") +
...
ggplot(df.scatter.plot, mapping = aes(x = labFinalScore, y = tosrecFinalScore)) + 
  geom_point(size = 1.5, color = "gray20") +
  geom_smooth(method = "lm", color="black") +
  facet_wrap(~labCorpus) +
  ...
  labs(x = "SRE Score (correct - incorrect)",
       ...
       color = "SRE Form") + 
  theme_light() +
  theme(text=element_text(family="Avenir", size=12), strip.text = element_text(colour = 'black'))
```
