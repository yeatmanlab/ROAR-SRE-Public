...
quad.plot <- ggplot(data = df.quad.plot, label = sentence, aes(label = sentence)) + 
  ...
  geom_label_repel(data = df.quad.plot %>% 
                   ...
                   fill = "gray95", family="Avenir", size = 2.5) +
  scale_x_continuous(limits = c(-3, 3), breaks = round(seq(-3,3)), "Average response time (s)") +
  scale_y_continuous(limits = c(-3, 3), breaks = round(seq(-3,3)),"Average agreement rate") +
...
```{r}
ggsave('figures/SRE-Pilot-PCorr-RT-Scatter.png', filtered.plot, height = 4, width = 7, dpi=300)
ggsave('figures/SRE-Pilot-Quad-Scatter.png', quad.plot, height = 4, width = 7, dpi=300)
ggsave('figures/SRE-Fixed-Pilot-Scatter.png', pilot.fixed.scatter, dpi=300)
```
